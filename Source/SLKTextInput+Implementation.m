...
- (NSString *)wordAtRange:(NSRange)range rangeInText:(NSRangePointer)rangePointer
{
    ...
    if ([leftWordPart hasPrefix:@"@"]) {
        *rangePointer = NSMakeRange(location-leftWordPart.length, leftWordPart.length);

        return leftWordPart;
    }
    NSInteger leftComponentsCount = [leftComponents count];
    if (leftComponentsCount > 1) {
        NSString *secondLeftWordPart = [leftComponents objectAtIndex:leftComponentsCount-2];
        if ([secondLeftWordPart hasPrefix:@"@"]) {
            // take last 2 left components
            NSString *word = [NSString stringWithFormat:@"%@ %@", secondLeftWordPart, leftWordPart];
            *rangePointer = NSMakeRange(location-word.length, word.length);
            return word;
        }
    }
